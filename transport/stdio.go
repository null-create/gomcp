...
// Based off of https://github.com/mark3labs/mcp-go/tree/main/client/transport
...
type Stdio struct {
	...
	onNotification func(codec.Notification)
...
func (c *Stdio) SetNotificationHandler(
	handler func(notification codec.Notification),
) {
	c.notifyMu.Lock()
...
func (c *Stdio) readResponses() {
	for {
		...
		default:
			...
			var baseMessage codec.JSONRPCResponse
			...
			// Handle notification
			if baseMessage.ID == 0 {
				var notification codec.Notification
