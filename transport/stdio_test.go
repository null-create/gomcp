...
func compileTestServer(outputPath string) error {
	cmd := exec.Command(
		...
		"-o",
		outputPath,
		"../server/test-server/mockserver.go",
	)
...
func TestStdio(t *testing.T) {
	...
	t.Run("SendRequest", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("Failed to encode test params: %v", err)
		}
	...
	t.Run("SendNotification & NotificationHandler", func(t *testing.T) {
		...
		notificationChan := make(chan codec.Notification, 1)

		// Set notification handler
		stdio.SetNotificationHandler(func(notification codec.Notification) {
		...
		p, err := json.Marshal(codec.Notification{
			JSONRPC: codec.JsonRPCVersion,
		})
		if err != nil {
			t.Fatalf("Failed to encode test notification: %v", err)
		}
		...
		go func() {
			...
			case nt := <-notificationChan:
				...
				responseJson, _ := json.Marshal(nt.AdditionalFields)
	...
	t.Run("MultipleRequests", func(t *testing.T) {
		...
		responses := make([]*codec.JSONRPCResponse, numRequests)
		...
		for i := range numRequests {
			...
			if responses[i] == nil || responses[i].ID == 0 || responses[i].ID != int64(100+i) {
			...
			if err := json.Unmarshal(responses[i].Result.([]byte), &result); err != nil {
	...
	t.Run("ResponseError", func(t *testing.T) {
		...
		var responseError codec.JSONRPCRequest
